...
const movies = [
  ...
  /* 
    List All Actors Write a function getAllActors(movies) that 
    returns an array of all actor names (no duplicates).
  */
  ...
  function getAllActors(movies) {
    ...
    for (var i=0; i< movies.length; i++) {
      for (var j=0; j< movies[i].actors.length; j++) {
        if (!actors.includes(movies[i].actors[j].name)) {
          actors.push(movies[i].actors[j].name)
        }
      }
    }
    return actors
