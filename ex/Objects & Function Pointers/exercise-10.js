...
/* 
    Total Salary per Movie 
    Write a function calculateTotalSalary(movies, movieId) 
    that calculates and returns the total salary paid to actors for a given movie 
    by its id.
  */

function calculateTotalSalary(movies, movieId) {
	var totalSalary = 0
	...
	for (var i = 0; i < movies.length; i++) {
        ...
        for (var j = 0; j < movies[i].actors.length; j++) {
            totalSalary += movies[i].actors[j].salary
        }
        return totalSalary
	}
}
...
console.log(calculateTotalSalary(movies, 7))
