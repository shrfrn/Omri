...
function getFrequentActors(movies) {
    ...
    for(var i = 0; i < movies.length; i++){
        for(var j = 0; j < movies[i].actors.length; j++){
            const currActorName = movies[i].actors[j].name

            if (frequentActors.includes(currActorName)) continue

            if (foundActors.includes(currActorName)) {
                frequentActors.push(currActorName)
            } else {
                foundActors.push(currActorName)
            }
        }
    }
...
// 					if (movies[k].actors[l].name === movies[i].actors[j].name && !frequentActors.includes(movies[i].actors[j].name)) frequentActors.push(movies[i].actors[j].name)
// 				}
// 			}
// 		}
// 	}
