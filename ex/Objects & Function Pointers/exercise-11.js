...
const movies = [
  ...
  /* 
    ...
    and returns the top n highest-earning actors based on their total salaries across all movies.
  */
  ...
  function getTopEarningActors(movies, n) {
    ...
    for (var i=0; i<movies.length; i++) {
      for (var j=0; j<movies[i].actors.length; j++) {
        ...
        topEarners.find(actor => actor.name === movies[i].actors[0].name).totalSalary += movies[i].actors[j].salary
        }
      }
    }
  ...
  console.log(getTopEarningActors(movies, 3))
