...
const movies = [
	{
		...
		year: 2010,
		actors: [
			{ name: 'Leonardo DiCaprio', salary: 5000000 },
...
/* 
    Find the Highest Paid Actor 
    Write a function getHighestPaidActor(movies) that finds and 
    returns the actor object with the highest salary across all movies.
  */

function getHighestPaidActor(movies) {
	var highestPaidActor = movies[0].actors[0]
    
	for (var i = 0; i < movies.length; i++) {
		for (var j = 0; j < movies[i].actors.length; j++) {
			if (movies[i].actors[j].salary > highestPaidActor.salary) {
                console.log(highestPaidActor.salary.toLocaleString())
				highestPaidActor = movies[i].actors[j]
			}
		}
	}
...
console.log(getHighestPaidActor(movies))
